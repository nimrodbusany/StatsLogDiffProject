...
from s2kdiff_experiment_results import S2KDiff_Experiment_Result
...
def run_model_based_experiments(ks, min_diffs, alphas, traces_to_sample, experiment_results,
                                results_folder, models_group=0, single_instance=False, repetitions= 10):
    ...
    for model_id in MODEL_ID:
        for (k, min_diff, alpha) in itertools.product(ks, min_diffs, alphas):
            logs, true_ksequence_transtion_probabilities, model_name, outpath = get_models(model_id, results_folder, k, models_group)
            if single_instance:
                        ...
                        for diff in diffs:
                            ...
                            if not item['significant_diff']:
                                ...
                                if item['true_diff'] < min_diff:
                                    item['statstical_success'] = False
                            item['k'] = k
                            item['min_diff'] = min_diff
                            item['alpha'] = alpha
                            item['sample_size'] = sample_size
                            item['trial'] = trial
                            item['model'] = model_name
                            item['l1_traces'] = sample_size
                            item['l2_traces'] = sample_size

                            df = df.append(item, ignore_index=True)
                        df.to_csv(outpath + 'ex_' + model_name + "_" + vals + '_diffs' + '.csv')
                        # print_diffs(diffs, RESULT_FODLER + 'ex_' + experiment_name + "_" + vals + '_diffs' + '.csv')
                    experiment_results.add_experiment_result(true_ksequence_transtion_probabilities, k, min_diff, -1, alpha, diffs, sample_size, sample_size, model_name, single_instance)
...
# if __name__ == '__main__':
    ...
    # experiment_setup_varying_instance()